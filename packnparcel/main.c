/*
Title: Pack and Parcel
Author: Prakash Hegade
Date: 12 Aug 2018

 - Pack N Parcel is a delivery agency.
 - The different kinds of delivery made are home delivery,through email and voice mails.
 - The program below explains the process and the automation.
...
int main()
{
    printf("Enter the input of 5 transactions\n");
    get_transactions();
    business();
    return 0;
}
    ...
    for(i = 0; i < 5; i++)
    {
      ...
      // Make a note of how various structure members are accessed
        ...
        if(p[i].delivery_type == 3)
        {
            ...
            scanf("%i%i%i", &p[i].d.v.code, &p[i].d.v.number_part1, &p[i].d.v.number_part2);
            p[i].charges = 200.00;
        }
    }
